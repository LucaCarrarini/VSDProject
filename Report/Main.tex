...
First of all, one defines a set of generalized coordinates $\bm{q} = [q_1, q_2, \dots, q_N]$ and the derivative $\dot{\bm{q}}$. This two variables describe in a unique way the motion of the system. The second step is to define the so--called Lagrangian
\begin{equation}
L(\bm{q},\dot{\bm{q}}) = T(\bm{q},\dot{\bm{q}}) - U(\bm{q})
\label{lagrangian}
\end{equation}
where $T(\cdot)$ represents the kinetic energy of the system, which depends only on the generalized velocities, and $U(\cdot)$ indicates the potential energy.
...
Using the lagrangian mechanics, the mathematical model of a KERS system has been built. This model needs to take into account the motion of the flywheel and the vehicle itself. Having this in mind, a straightforward choice is to pick $\omega_f$ and $\omega_w$ as generalized coordinates. In fact the system can be fully described using these two quantities, since the angular velocities of the other components can be obtained starting from the aforementioned two with some manipulations involving the transmission ratios and the efficiency. 

Due to the hypothesis of flat road, the whole system does not have either mechanical potential energy. So the lagrangian is given by the two kinetic energy contibutions:
\begin{equation}
